...
export const Route = createFileRoute('/homeBackup')({
  component: RouteComponent,
})

const categoryDetails: Record<string, { title: ReactElement, subtitle: ReactElement }> = {
    "Overview": {
        "title": <span className='font-bold text-[6cqh]  leading-none tracking-[-1.816px]'>Track, See & Estimate Economic Risk</span>,
        "subtitle": <span className='tracking-[-1.089px] text-[2cqh] '><b>Real-time monitoring</b> of floods, hurricanes, wildfires and geopolitical events — and exactly what they put in harm's way.</span>
    },
    "Track": {
        "title": <span className='font-bold text-[6cqh] leading-none tracking-[-1.816px]'>Track Economic Risk</span>,
        "subtitle": <span className='tracking-[-1.089px] text-[2cqh] '>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.</span>
    },
    "See": {
        "title": <span className='font-bold text-[6cqh] leading-none tracking-[-1.816px]'>See Forward-Looking Risk</span>,
        "subtitle": <span className='tracking-[-1.089px] text-[2cqh] '>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.</span>
    },
    "Estimate": {
        "title": <span className='font-bold text-[6cqh] leading-none tracking-[-1.816px]'>Estimate Gridded Economies</span>,
        "subtitle": <span className='tracking-[-1.089px] text-[2cqh] '>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.</span>
    },
}
...
function RouteComponent() {
    ...
    return <div className=''>
        <div className='h-full relative overflow-hidden pt-45 pb-16 flex flex-col justify-start items-center xl:justify-normal xl:items-start w-full bg-fixed bg-cover' style={{ backgroundImage: `url(${MenuBackground})`, backgroundPositionY: "bottom 10px" }}>
            <div className='flex flex-col lg:flex-row items-center'>
                <div className='w-8/10 lg:w-4/10 flex justify-center '>
                    <div className='w-9/10  grid text-white xl:text-left z-1'>
                        {Object.entries(categoryDetails).map(([key, details]) =>
                            <div
                                key={key}
                                className={`col-start-1 row-start-1 flex flex-col gap-y-4 text-left ${key === activeCategory ? '' : 'invisible'}`}
                            >
                                {details.title}
                                {details.subtitle}
                            </div>
                ...
                <img className='w-full lg:w-6/10' src={globeExtrusions}></img>
            </div> 
            <div className="relative grid md:grid-cols-2 xl:grid-cols-4 justify-center xl:w-full max-w-320 gap-5 z-1 xl:pl-15">
                {cardDetails.map((e, i) =>
                    <div
                        key={i}
